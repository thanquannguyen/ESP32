...
#define SAMPLE_DEVICE_NAME "NHOM4"
...
static uint8_t raw_adv_data[] = {
    ...
    // /* device name */
    // 0x06, 0x09, 'N', 'H', 'O', 'M', '4'
    0x53, 0x61, 0x6C, 0x65, 0x20, 0x6F, 0x66, 0x66, 0x20, 0x35, 0x30, 0x25};
...
void task_ssd1306_display_hex(void *arg_data)
{
    uint8_t *data = (uint8_t *)arg_data;
    size_t len = strlen((const char *)data) / 2; // Calculate the length of the data array

    char text[len * 2 + 1]; // Allocate space for the text representation of the hex number
    for (size_t i = 0; i < len; i++)
    {
        sprintf(text + i * 2, "%02X", data[i]); // Convert each byte to a two-digit hex string
    }
    text[len * 2] = '\0'; // Null-terminate the string

    task_ssd1306_display_text(text); // Display the hex string using the existing text display function

    vTaskDelete(NULL); // Delete this task after it has completed
}

uint8_t write_data[20] = {0x00};
int pos = 0;
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_WRITE_EVT:
        if (!param->write.is_prep)
        {
            ...
            {
                uint8_t write_data0[4] = {0x20, 0x52, 0x16, 0x82};
                uint8_t write_data1[4] = {0x20, 0x52, 0x17, 0x57};
                uint8_t write_data2[4] = {0x20, 0x52, 0x14, 0x97};
                uint8_t write_data3[4] = {0x20, 0x52, 0x09, 0x03};
                // uint8_t write_data2[4] = {0x20, 0x52};
                // uint8_t write_data3[4] = {0x20, 0x52};
                if (memcmp(write_data0, param->write.value, param->write.len) == 0)
                {
                    ESP_LOGI(EXAMPLE_TAG, "(3)***** short write success ***** \n");
                    ESP_LOGI(EXAMPLE_TAG, "%x%x%x%x", write_data0[0], write_data0[1], write_data0[2], write_data0[3]);
                    memcpy(&write_data[pos], write_data0, sizeof(write_data0));
                    pos += sizeof(write_data0);
                    write_data[pos++] = '\n';
                }
                if (memcmp(write_data1, param->write.value, param->write.len) == 0)
                {
                    ESP_LOGI(EXAMPLE_TAG, "(3)***** short write success ***** \n");
                    ESP_LOGI(EXAMPLE_TAG, "%x%x%x%x", write_data1[0], write_data1[1], write_data1[2], write_data1[3]);
                    memcpy(&write_data[pos], write_data1, sizeof(write_data1));
                    pos += sizeof(write_data1);
                    write_data[pos++] = '\n';
                }
                if (memcmp(write_data2, param->write.value, param->write.len) == 0)
                {
                    ESP_LOGI(EXAMPLE_TAG, "(3)***** short write success ***** \n");
                    ESP_LOGI(EXAMPLE_TAG, "%x%x%x%x", write_data2[0], write_data2[1], write_data2[2], write_data2[3]);
                    memcpy(&write_data[pos], write_data2, sizeof(write_data2));
                    pos += sizeof(write_data2);
                    write_data[pos++] = '\n';
                }
                if (memcmp(write_data3, param->write.value, param->write.len) == 0)
                {
                    ESP_LOGI(EXAMPLE_TAG, "(3)***** short write success ***** \n");
                    ESP_LOGI(EXAMPLE_TAG, "%x%x%x%x", write_data3[0], write_data3[1], write_data3[2], write_data3[3]);
                    memcpy(&write_data[pos], write_data3, sizeof(write_data3));
                    pos += sizeof(write_data3);
                    write_data[pos++] = '\n';
                }
                for (int i = 0; i < sizeof(write_data); i++)
            {
                    ...
                    {
                        ESP_LOGI(EXAMPLE_TAG, "%02x", write_data[i]);
                    }
                    // ESP_LOGI(EXAMPLE_TAG, "%02x", write_data[i]);
                }
                size_t num_numbers = sizeof(write_data) / sizeof(write_data[0]);
                char str[num_numbers * 2 + 1]; // Reserve space for the string, including the null terminator
                for (size_t i = 0; i < num_numbers; i++)
                {
                    ...
                    {
                        sprintf(str + i * 2, "%02X", write_data[i]); // Convert each number to a two-digit hex string
                    }
                }
                str[num_numbers * 2] = '\0'; // Null-terminate the string
                ESP_LOGI(EXAMPLE_TAG, "%s", str);
                
                xTaskCreate(&task_ssd1306_display_clear, "ssd1306_display_clear", 2048, NULL, 6, NULL);
                vTaskDelay(100 / portTICK_PERIOD_MS);
                xTaskCreate(&task_ssd1306_display_text, "ssd1306_display_text", 2048, (void *)str, 6, NULL);
                // xTaskCreate(&task_ssd1306_display_hex, "ssd1306_display_hex", 2048, (void *)write_data, 6, NULL);
                vTaskDelay(100 / portTICK_PERIOD_MS);
            }
...
void app_main(void)
{
    ...
    // xTaskCreate(&task_ssd1306_display_clear, "ssd1306_display_clear", 2048, NULL, 6, NULL);
    // vTaskDelay(100 / portTICK_PERIOD_MS);
    // xTaskCreate(&task_ssd1306_display_text, "ssd1306_display_text", 2048, str, 6, NULL);
    // // xTaskCreate(&task_ssd1306_display_hex, "ssd1306_display_hex", 2048, (void *)write_data, 6, NULL);
    // vTaskDelay(100 / portTICK_PERIOD_MS);
}
